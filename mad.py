...
GPIO.add_event_detect(22, GPIO.BOTH, callback=selectButton, bouncetime=2000)
...
def PROGRAM_BREW():
    if drawBrewHud:
        ...
        exec(open('/media/pi/PROGRAM/programs/' +
             brewSoftList[brewListPos] + '.py').read())
        print(brewSoftList[brewListPos])
        # FIRM program - Displays the firmware of the M.A.D.
...
def PROGRAM_FIRMWARE():
    ...
    lcd.cursor_pos = (1, 0)
    lcd.write_string(" REVSION  1.0.0 ")
...
def PROGRAM_SYSINFO():
    ...
    lcd.write_string("--    INFO    --")
    lcd.cursor_pos = (1, 0)
    lcd.write_string("SYSTEM OS: 1.0.0")
